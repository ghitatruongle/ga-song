...
class _FrameBudgetOverlayState extends State<FrameBudgetOverlay>
  ...
  void _updateDisplay() {
    ...
    if (_frameTimes.length >= 2) {
      ...
      if (frameDeltas.isNotEmpty) {
        frameDeltas.sort();
        final avgUs = frameDeltas.reduce((a, b) => a + b) / frameDeltas.length;
        final p99Us = frameDeltas[(frameDeltas.length * 0.99).floor()];
        final jankFrames = frameDeltas.where((d) => d > 16000).length; // > 16ms
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Icon(
                        _getFpsColor(_fps) == Colors.green ? Icons.check_circle : Icons.warning,
                  ...
                  _buildMetric('Avg Frame', '${_avgFrameMs}ms', _getFrameTimeColor(_avgFrameMs)),
                  _buildMetric('P99 Frame', '${_p99FrameMs}ms', _getFrameTimeColor(_p99FrameMs)),
                  _buildMetric('Jank (>16ms)', _jankCount, _jankCount != '--' && int.tryParse(_jankCount)! > 5 ? Colors.orange : Colors.green),
  ...
  Color _getFrameTimeColor(String ms) {
