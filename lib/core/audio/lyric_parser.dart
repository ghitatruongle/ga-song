...
class LyricLine {
  ...
  LyricSyllable? getSyllableAt(Duration position) {
  ...
  int getSyllableIndexAt(Duration position) {
...
class LyricParser {
  ...
  static bool _isEnhancedLrc(String line, Iterable<RegExpMatch> matches) {
    // Enhanced LRC has timestamps both at the start of the line AND within the text
    ...
    final betweenText = line.substring(secondLastMatch.end, lastMatch.start).trim();
  ...
  static LyricLine _parseEnhancedLrcLine(String line, Iterable<RegExpMatch> matches) {
    final matchList = matches.toList();
    ...
    for (int i = 0; i < matchList.length - 1; i++) {
      ...
      final syllableText = line.substring(currentMatch.end, nextMatch.start).trim();
